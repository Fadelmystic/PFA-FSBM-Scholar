...
function createModeSelector() {
    ...
    ragBtn?.addEventListener('click', () => switchMode('rag'));
    kgBtn?.addEventListener('click', () => switchMode('kg'));

    return modeSelector;
}

// Fonction pour changer de mode
function switchMode(mode: 'rag' | 'kg') {
    currentMode = mode;
...
async function sendMessageFooter() {
    const messageInput = document.getElementById('messageInputFooter') as HTMLTextAreaElement;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (currentMode === 'rag') {
                ...
                if (data.entities_found && data.entities_found.length > 0) {
                    ...
                    data.entities_found.forEach((entity: any) => {
        ...
        showTypingIndicator(false);
        if(sendMsgBtnFooter) sendMsgBtnFooter.disabled = false;
    }
}
...
function startNewChat() {
    ...
    currentChatId = null;
    if (currentChatTitleHeader) currentChatTitleHeader.textContent = "Nouveau Chat FSBM Scholar Assistant";
    clearChatArea();
    ...
    setTimeout(() => {
        const welcomeMessage = createMessageBubble(
            `Bonjour ! Je suis votre assistant FSBM Scholar Assistant. 🎓\n\n` +
            `**Mode actuel : RAG** 🔍\n` +
            `Je peux rechercher dans vos documents PDF et répondre à vos questions.\n\n` +
            `**Choisissez votre mode d'assistance ci-dessus :**\n` +
            `• **RAG** : Recherche dans vos documents\n` +
            `• **KG** : Graphe de connaissances\n\n` +
            `Posez-moi vos questions sur Java, bases de données, architecture, etc. !`,
            'bot'
        );
