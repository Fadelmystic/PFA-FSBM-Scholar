...
const mainAreaMobileSidebarToggleBtn = document.getElementById('mainAreaMobileSidebarToggleBtn');
...
const sendMsgBtnFooter = document.getElementById('sendMsgBtnFooter');
const uploadFileBtnFooter = document.getElementById('uploadFileBtnFooter');
const fileInputFooter = document.getElementById('fileInputFooter');
const currentChatTitleHeader = document.getElementById('currentChatTitleHeader');
const userProfileContainer = document.getElementById('userProfileContainer');
const userAccountDropdown = document.getElementById('userAccountDropdown');
const userNameDisplay = document.getElementById('userNameDisplay');
const userAvatar = document.getElementById('userAvatar');
const logoutChatBtn = document.getElementById('logoutChatBtn');
...
let currentMode = 'rag';
// Fonction pour créer le sélecteur de mode
function createModeSelector() {
    const modeSelector = document.createElement('div');
    modeSelector.className = 'mode-selector mb-4 p-3 bg-gray-50 rounded-lg';
    modeSelector.innerHTML = `
        <div class="flex items-center space-x-4">
            <span class="text-sm font-medium text-gray-700">Mode d'assistance :</span>
            <div class="flex space-x-2">
                <button id="ragMode" class="mode-btn px-3 py-1 rounded-md text-sm font-medium bg-blue-100 text-blue-800 border border-blue-200">
                    🔍 RAG (Documents)
                </button>
                <button id="kgMode" class="mode-btn px-3 py-1 rounded-md text-sm font-medium text-gray-600 border border-gray-200">
                    🧠 KG (Graphe)
                </button>
            </div>
        </div>
        <p class="text-xs text-gray-500 mt-2">
            <span id="modeDescription">RAG : Recherche dans vos documents PDF avec IA</span>
        </p>
    `;
    // Ajouter les événements
    const ragBtn = modeSelector.querySelector('#ragMode');
    const kgBtn = modeSelector.querySelector('#kgMode');
    const modeDesc = modeSelector.querySelector('#modeDescription');
    ragBtn === null || ragBtn === void 0 ? void 0 : ragBtn.addEventListener('click', () => switchMode('rag'));
    kgBtn === null || kgBtn === void 0 ? void 0 : kgBtn.addEventListener('click', () => switchMode('kg'));
    return modeSelector;
}
...
function switchMode(mode) {
    ...
    const ragBtn = document.getElementById('ragMode');
    const kgBtn = document.getElementById('kgMode');
    const modeDesc = document.getElementById('modeDescription');
    if (ragBtn && kgBtn && modeDesc) {
        if (mode === 'rag') {
            ragBtn.className = 'mode-btn px-3 py-1 rounded-md text-sm font-medium bg-blue-100 text-blue-800 border border-blue-200';
            kgBtn.className = 'mode-btn px-3 py-1 rounded-md text-sm font-medium text-gray-600 border border-gray-200';
            modeDesc.textContent = 'RAG : Recherche dans vos documents PDF avec IA';
        }
        else {
            kgBtn.className = 'mode-btn px-3 py-1 rounded-md text-sm font-medium bg-green-100 text-green-800 border border-green-200';
            ragBtn.className = 'mode-btn px-3 py-1 rounded-md text-sm font-medium text-gray-600 border border-gray-200';
            modeDesc.textContent = 'KG : Graphe de connaissances avec entités et relations';
        }
    }
    // Afficher le mode actuel
    console.log(`Mode changé vers: ${mode.toUpperCase()}`);
}
// --- Helper Functions ---
...
function createMessageBubble(message, sender) {
    ...
    bubbleDiv.classList.add('p-3', 'rounded-xl', 'max-w-xs', 'sm:max-w-sm', 'md:max-w-md', 'lg:max-w-lg', 'text-sm', 'shadow-md', 'break-words', sender === 'user' ? 'message-bubble-user' : 'message-bubble-bot');
...
function clearChatArea() {
    const messagesInnerContainer = messageDisplayArea === null || messageDisplayArea === void 0 ? void 0 : messageDisplayArea.querySelector('.max-w-3xl');
    if (messagesInnerContainer) {
...
// Modifier la fonction sendMessageFooter pour supporter les deux modes
function sendMessageFooter() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        messageInput.value = '';
        if (sendMsgBtnFooter)
            sendMsgBtnFooter.disabled = true;
        // Afficher l'indicateur de frappe
        showTypingIndicator(true);
        // Déterminer l'URL selon le mode
        const apiUrl = currentMode === 'rag' ? 'http://localhost:8000/chat' : 'http://localhost:8001/chat';
        const modeName = currentMode === 'rag' ? 'RAG' : 'KG';
        console.log(`Envoi vers ${modeName} (${apiUrl}): "${messageText}"`);
        try {
            ...
            const response = yield fetch(apiUrl, {
                ...
                body: JSON.stringify({
                    message: messageText,
                    similarity_threshold: currentMode === 'rag' ? 0.35 : undefined,
                    max_docs: currentMode === 'rag' ? 5 : undefined
                }),
            ...
            if (response.ok) {
                ...
                if (currentMode === 'rag') {
                    ...
                    if (data.entities_found && data.entities_found.length > 0) {
                        botResponse += `\n\n🔍 **Entités trouvées:** ${data.entities_found.length}`;
            ...
            let errorMessage = `Désolé, une erreur est survenue lors de la communication avec l'assistant ${modeName}.`;
            if (error instanceof TypeError && error.message.includes('fetch')) {
                errorMessage += ` Problème de connexion. Vérifiez que l'API ${modeName} est démarrée sur le port ${currentMode === 'rag' ? '8000' : '8001'}.`;
            }
            else if (error.name === 'AbortError') {
                errorMessage = `La réponse prend trop de temps (timeout). Veuillez réessayer avec une question plus simple.`;
            }
            else if (error.message.includes('timeout')) {
                errorMessage += ` La réponse prend trop de temps. Veuillez réessayer.`;
            }
            else {
                errorMessage += ` Veuillez réessayer dans quelques secondes.`;
            }
            const errorBubble = createMessageBubble(errorMessage, 'bot');
            displayMessage(errorBubble);
        }
        finally {
            showTypingIndicator(false);
            if (sendMsgBtnFooter)
                sendMsgBtnFooter.disabled = false;
        }
    });
}
function loadChat(chatId, chatName) {
    ...
    if (currentChatTitleHeader)
        currentChatTitleHeader.textContent = chatName;
    clearChatArea();
    // Ajouter le sélecteur de mode
    const modeSelector = createModeSelector();
    const chatContainer = document.getElementById('chatContainer');
    if (chatContainer) {
        chatContainer.appendChild(modeSelector);
    }
    getTypingIndicator();
    showTypingIndicator(true);
    setTimeout(() => {
        const welcomeMessages = [
            createMessageBubble(`FSBM Scholar Assistant: Bienvenue dans la conversation "${chatName}". Je suis votre assistant spécialisé en informatique et bases de données.`, 'bot'),
            createMessageBubble(`**Mode actuel : RAG** 🔍\n` +
                `Je peux rechercher dans vos documents PDF et répondre à vos questions.\n\n` +
                `**Choisissez votre mode d'assistance ci-dessus :**\n` +
                `• **RAG** : Recherche dans vos documents\n` +
                `• **KG** : Graphe de connaissances`, 'bot')
        ];
        welcomeMessages.forEach(displayMessage);
        console.log("Welcome messages loaded for chat:", chatName);
    }, 800);
...
function startNewChat() {
    console.log("Starting new FSBM Scholar Assistant chat");
    currentChatId = null;
    if (currentChatTitleHeader)
        currentChatTitleHeader.textContent = "Nouveau Chat FSBM Scholar Assistant";
    clearChatArea();
    // Ajouter le sélecteur de mode
    const modeSelector = createModeSelector();
    const chatContainer = document.getElementById('chatContainer');
    ...
    setTimeout(() => {
        const welcomeMessage = createMessageBubble(`Bonjour ! Je suis votre assistant FSBM Scholar Assistant. 🎓\n\n` +
            `**Mode actuel : RAG** 🔍\n` +
            `Je peux rechercher dans vos documents PDF et répondre à vos questions.\n\n` +
            `**Choisissez votre mode d'assistance ci-dessus :**\n` +
            `• **RAG** : Recherche dans vos documents\n` +
            `• **KG** : Graphe de connaissances\n\n` +
            `Posez-moi vos questions sur Java, bases de données, architecture, etc. !`, 'bot');
...
if (sendMsgBtnFooter)
    sendMsgBtnFooter.addEventListener('click', sendMessageFooter);
...
function setSidebarState(expanded) {
    if (!sidebar || !sidebarToggleBtn || !mainChatArea)
        return;
    ...
    if (mobileSidebarOverlay)
        mobileSidebarOverlay.classList.toggle('hidden', !expanded || !isMobile);
...
if (sidebarToggleBtn)
    sidebarToggleBtn.addEventListener('click', () => setSidebarState(!isSidebarProgrammaticallyOpen));
if (mainAreaMobileSidebarToggleBtn)
    mainAreaMobileSidebarToggleBtn.addEventListener('click', () => { if (window.innerWidth < 768)
        setSidebarState(true); });
if (mobileSidebarCloseBtn)
    mobileSidebarCloseBtn.addEventListener('click', () => { if (window.innerWidth < 768)
        setSidebarState(false); });
if (mobileSidebarOverlay)
    mobileSidebarOverlay.addEventListener('click', () => { if (window.innerWidth < 768)
        setSidebarState(false); });
function setHistorySectionState(open) {
    if (!historySection || !historyArrowIcon || !historyToggleBtn)
        return;
...
if (userProfileContainer && userAccountDropdown) {
    ...
    document.addEventListener('click', (event) => {
        if (!userAccountDropdown.classList.contains('hidden') &&
            ...
            !userAccountDropdown.contains(event.target)) {
            userAccountDropdown.classList.add('hidden');
        }
    });
    if (logoutChatBtn)
        logoutChatBtn.addEventListener('click', (e) => {
            e.preventDefault();
    ...
    chatsData.forEach(chat => {
        ...
        dropdownMenu.querySelectorAll('.history-dropdown-action').forEach(actionLink => {
            actionLink.addEventListener('click', (e) => {
                ...
                const currentName = ((_a = chatsData.find(c => c.id === chatId)) === null || _a === void 0 ? void 0 : _a.name) || chat.name; // Get current name from data
...
function handleHistoryAction(action, chatId, chatName, itemElement) {
    if (!action || !chatId || !chatName)
        return;
    ...
    switch (action) {
        case 'rename':
            ...
            if (newName && newName.trim() !== "" && newName !== chatName) {
                ...
                const itemLink = itemElement.querySelector('.history-item');
                if (itemLink)
                    itemLink.dataset.chatName = newName; // Update dataset for future actions
...
if (searchConversationsInput && conversationHistoryList) {
    searchConversationsInput.addEventListener('input', () => {
        ...
        const items = conversationHistoryList.querySelectorAll('.history-item-container'); // Search within containers
        let visibleItemsCount = 0;
        items.forEach(itemContainer => {
            var _a;
            const itemLink = itemContainer.querySelector('a.history-item');
            if (itemLink) {
                const name = ((_a = itemLink.dataset.chatName) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
        ...
        let noResultsEl = conversationHistoryList.querySelector('.no-results-message');
...
function initializeChatInterface() {
    var _a, _b, _c;
    const userData = { name: "Utilisateur FSBM Scholar", avatarUrl: null };
    if (userNameDisplay)
    ...
    if (userAvatar) {
        if (userData.avatarUrl) {
            ...
            const initials = (((_a = nameParts[0]) === null || _a === void 0 ? void 0 : _a[0]) || '') + (nameParts.length > 1 ? ((_b = nameParts[1]) === null || _b === void 0 ? void 0 : _b[0]) || '' : (((_c = nameParts[0]) === null || _c === void 0 ? void 0 : _c.length) > 1 ? nameParts[0][1] : ''));
    ...
    startNewChat();
    console.log("FSBM Scholar Chat interface initialized (With RAG + KG modes).");
    ...
    document.addEventListener('click', (e) => {
        ...
        openDropdowns.forEach(dropdown => {
            ...
            const optionsBtn = (_a = dropdown.previousElementSibling) === null || _a === void 0 ? void 0 : _a.querySelector('.history-item-options-btn');
            if (!dropdown.contains(e.target) && (optionsBtn && !optionsBtn.contains(e.target))) {
...
window.addEventListener('resize', () => {
    ...
    const currentMobileState = sidebar === null || sidebar === void 0 ? void 0 : sidebar.classList.contains('is-mobile-visible');
    const currentDesktopState = sidebar === null || sidebar === void 0 ? void 0 : sidebar.classList.contains('is-expanded-desktop');
    if (isDesktop) {
